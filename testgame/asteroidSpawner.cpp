...
const uint8_t AsteroidSpawner::SECONDS_BETWEEN_ASTEROID_SPAWN = 300;
...
void AsteroidSpawner::onUpdate(float elapsedTime) {
    ...
    if (asteroids->size() == 0) {
        asteroids->push_back(new Asteroid(gameEngine, 50, 500, 90));
        asteroids->push_back(new Asteroid(gameEngine, 1900, 500, 270));
    }
