#ifndef asteroidsGame_hpp
#define asteroidsGame_hpp

#include "gameEngine.hpp"
#include "sprite.hpp"
#include "animatedGraphic.hpp"
#include "asteroid.hpp"
#include "bullet.hpp"
#include "ship.hpp"
#include "asteroidSpawner.hpp"

class AsteroidsGame : public jimp::GameEngine {

private:
    jimp::Sprite* background = nullptr;
    AsteroidSpawner* asteroidSpawner = nullptr;
    jimp::Sound* music = nullptr;

public:
    AsteroidsGame(int screenWidth, int screenHeight, std::string name) : GameEngine(screenWidth, screenHeight, 0, name, 60, false) {
        registerGraphic(std::make_unique<Ship>());
        music = createNewSound("music.ogg");
        music->loop(25);
        asteroidSpawner = new AsteroidSpawner();
        background = new jimp::Sprite(0, 0, 1.0F, 0, 1, "background.jpeg");
    }

    ~AsteroidsGame() {
        delete background;
        delete asteroidSpawner;
    }

    void startGame() {
        start();
    }

    std::unique_ptr<jimp::StageFactory> getStageFactory() override {
        return nullptr;
    }

    void onFrame(float elapsedTime) override {
        draw(background);
    }

    void onUpdate(float elapsedTime) override {
        asteroidSpawner->onUpdate(elapsedTime);
    }
};

#endif
